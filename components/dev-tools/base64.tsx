...
import { ArrowLeft, Copy, Check, Upload, Download } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Textarea } from "@/components/ui/textarea"
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Card, CardContent } from "@/components/ui/card"
import { Alert, AlertDescription } from "@/components/ui/alert"
import { encodeBase64, decodeBase64, trackToolUsage } from "@/lib/dev-tools"
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center space-x-2">
        ...
        <h1 className="text-2xl font-bold">Base64编解码</h1>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg font-medium">{mode === "encode" ? "输入文本" : "输入Base64"}</h2>
                </div>
          ...
          <Card>
            <CardContent className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  {mode === "encode" ? <Download className="h-5 w-5" /> : <Upload className="h-5 w-5" />}
                  <h2 className="text-lg font-medium">{mode === "encode" ? "Base64结果" : "解码结果"}</h2>
                </div>
